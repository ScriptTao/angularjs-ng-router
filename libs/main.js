...
Array.prototype.maxFn = function () {
    var max = this[0];
    for (var i = 0; i < this.length; i++) {
        if (max < this[i]) {
            max = this[i]
...
function loading() {
    this.open = function () {
        ...
        el.classList.add('loading_shadow');
        el.innerHTML = '<div class="loading-style-15">\n' +
            '                <span></span>\n' +
            '                <span></span>\n' +
            '                <span></span>\n' +
            '                <span></span>\n' +
    ...
    this.close = function () {
        let ht = document.getElementsByClassName('loading_shadow')[0]
        if (ht) {
